...
**Änderungen (Deutsch):**
- **Feature**: Die Berechnung der Mitarbeiter-Lohnkosten und Produktion in den Firmenkarten wurde komplett überarbeitet. PROST nutzt (falls hinterlegt) nun deinen API-Key, um die **exakten** Energie-Regenerationsraten jedes einzelnen Mitarbeiters über die offizielle WarEra-API abzufragen (inkl. VIP-Boni).
- **Bug Fix**: Ein massiver Mathe-Fehler (Faktor 10 zu niedrig), der den täglichen Output und Lohn der Mitarbeiter völlig falsch eingeschätzt hat, wurde behoben. Die Lohnkosten und die "Lager voll"-Schätzungen sind jetzt extrem präzise.
- **UI Update**: Der "Storage Full"-Tooltip (beim Hover über den vollen Balken) schlüsselt nun genau auf, wie viele Punkte pro Tag von der Maschine und wie viele von den Mitarbeitern kommen.

**Changes (English):**
- **Feature**: Worker wage and production calculations in company cards have been completely overhauled. If an API key is provided, PROST now fetches the **exact** energy regeneration rates of each individual worker via the official WarEra API (including VIP bonuses).
- **Bug Fix**: Fixed a massive math error (factor 10 too low) that caused daily output and wages to be heavily underestimated. Wage costs and "Storage full" estimates are now extremely precise.
- **UI Update**: The "Storage Full" tooltip now provides an exact breakdown of how many points per day are generated by the engine versus your workers.
...
**Änderungen (Deutsch):**
- **Direkt-Update-Button**: Im Einstellungs-Update-Banner gibt es jetzt einen großen, grünen Update-Button `🔄 Direkt-Update`, der sofort die Tampermonkey-Aktualisierung öffnet.
- **Fokus-Erkennung & Auto-Reload**: Sobald ein Update angestoßen wurde und man wieder zurück zum Spiel-Tab wechselt, lädt sich das Spiel automatisch neu, um die neue Version sofort zu aktivieren.
...
**Changes (English):**
- **Direct Update Button**: The settings update banner now includes a styled, green `🔄 Direct Update` button that opens the Tampermonkey update installer page immediately.
- **Focus Detection & Auto-Reload**: Initiating an update and returning to the game tab triggers an automatic reload so the new version is loaded instantly.
